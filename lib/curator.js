...
bedrock.events.on('bedrock.init', function() {
  ...
  jsonld.documentLoader = didio.createDocumentLoader({
    ...
    baseUrl: bedrock.config['credential-curator']['authorization-io'].baseUrl
  });
  jsigs = jsigs();
...
function _composeIdentity(req, res, next) {
  ...
  async.auto({
    ...
    emit: ['compose', 'verifyKeyCredential', function(callback, results) {
      var resources = results.compose.credential.map(function(credential) {
      ...
      bedrock.events.emit(
        'bedrock-credential-curator.credential.CredentialQuery',
        {
          ...
          claimId: results.compose.credential[0]['@graph'].claim.id,
          resource: resources,
          actor: identity.id
        }, callback);
    }],
