...
function _composeIdentity(req, res, next) {
  ...
  if(identity && identity.id !== req.user.identity.id) {
    return next(new BedrockError(
      'Authentication mismatch. Credential query identity does not match ' +
  ...
  async.auto({
    ...
    getKey: function(callback) {
      ...
      callback(null, {
        id:
          bedrock.config['credential-curator'].credentialSigningPublicKey.id,
        privateKey: {
          privateKeyPem:
            bedrock.config['credential-curator'].credentialSigningPrivateKey
        }
      });
    },
    ...
    registerKeyCredential: [
      'compose', 'getKey', 'verifyKeyCredential', function(callback, results) {
      ...
      _registerKey({
        ...
        if(err && err.name === 'MaximumKeyRegistrationAttemptsExceeded') {
...
function _registerKey(options, callback) {
  // TODO: consider adding some portion of this to did-io

  var curatorConfig = bedrock.config['credential-curator'];
  ...
  if(!('attempts' in options)) {
    ...
    return callback(new BedrockError(
      'Duplicate credential.', 'MaximumKeyRegistrationAttemptsExceeded'));
  }
  ...
  async.auto({
    ...
    update: ['getCurrent', function(callback, results) {
      ...
      doc['urn:webdht:updateCounter'] = new BigNumber(updateCounter).plus(1);
      ...
      async.auto({
        ...
        post: ['sign', function(callback, results) {
          request.post(
            aioBaseUrl + doc.id, results.sign, function(err, response) {
            if(err) {
              return callback(err);
            }
    ...
    callback(err, err ? null : results.update);
  });
}
