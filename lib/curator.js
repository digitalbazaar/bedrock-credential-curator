...
bedrock.events.on('bedrock-express.configure.routes', function(app) {
  ...
  app.post(
    ...
    /*
     * pubicKey has the following properties
     * type: array that always contains 'CryptographicKey' and can also contain
    ...
    if(publicKey.owner !== req.user.identity.id) {
      // the identity referenced in the credential query does not correspond
      // to the authenticated user.  Log-out the current user.
      // TODO: we need to queue the request for handling after logout
      // TODO: we could optimize by not redirecting to logout but instead
      // making a call to clear the session and then redirecting to
      // login w/some way of indicating that a request is queued
      return res.redirect('/session/logout');
    }
    async.auto({
      ...
      display: ['compose', function(callback, results) {
        getDefaultViewVars(req, function(err, vars) {
          ...
          vars.curator = {
            identity: results.compose,
            query: credentialQuery
          };
          // TODO: move request into `curator`
          vars.request = {
  ...
  app.post(
    ...
    parseForm, parseJsonData, function(req, res, next) {
      ...
      // TODO: we need to frame the credential first
      if(req.body.credential[0]['@graph'].claim.id !== req.user.identity.id) {
        // the identity referenced in the credential does not correspond
        // to the authenticated user.  Log-out the current user.
        return res.redirect('/session/logout');
      }
      ...
      getDefaultViewVars(req, function(err, vars) {
        ...
        vars.curator = {
          identity: req.body,
          request: {storageCallback: req.query.storageCallback}
        };
        res.render('main.html', vars);
      });
    });
