...
function _composeIdentity(req, res, next) {
  ...
  var publicKey;
  if(identity) {
    console.log('XXX identity', JSON.stringify(identity, null, 2));
    const credentials = jsonld.getValues(identity, 'credential').map(
      credential => credential['@graph']);
    credential = credentials[0];
    if(!credential ||
      !jsonld.hasValue(credential, 'type', 'CryptographicKeyCredential')) {
  ...
  async.auto({
    compose: function(callback) {
      api.compose(
        req.user.identity, req.user.identity.id, credentialQuery, callback);
    },
    ...
    addKeyCredential: ['registerKey', function(callback, results) {
...
function _registerKey(options, done) {
  ...
  async.auto({
    ...
    update: ['getCurrent', function(callback, results) {
      ...
      var id;
      while(!id) {
        id = options.identity.id + '/keys/' + uuid();
...
function _storeCredentials(req, res, next) {
  ...
  async.auto({
    store: function(callback) {
      ...
      store.insert(req.user.identity, credential, function(err, record) {
...
function _checkDomain(domain, options, callback) {
  const parsed = URL.parse(config.server.baseUri);
  const domains = ['https://' + parsed.host, parsed.host];
  callback(null, domains.includes(domain));
}
